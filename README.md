...
- **Customizable filename and export format** with multi-language support

### Multi-Language Formatting System

Supports multiple numbering styles for volumes and chapters:
- Arabic numerals: {num}, {en.Volume}, {en.Chapter}
- Chinese numerals (upper): {cn.up.Volume}, {cn.up.Chapter}
- Chinese numerals (lower): {cn.low.Volume}, {cn.low.Chapter}
- Chinese numerals (mixed): {cn.num.Volume}, {cn.num.Chapter}
- Japanese numerals: {jp.Volume}, {jp.Chapter}

Other format variables:
- {name}: Chapter/volume name
- {word_count}: Word count

Special feature: Underscores `_` between language format variables and {name} are automatically converted to middle dots `·`

### Real-Time Preview

Each format configuration has its own:
- Help text explaining available variables
- Real-time preview in white bold text
- Customizable preview color and font size
- Live updates as you edit the format
...
支持卷和章节的多种编号样式：
- 阿拉伯数字：{num}、{en.Volume}、{en.Chapter}
- 中文数字大写：{cn.up.Volume}、{cn.up.Chapter}
- 中文数字小写：{cn.low.Volume}、{cn.low.Chapter}
- 中文数字混合：{cn.num.Volume}、{cn.num.Chapter}
- 日文数字：{jp.Volume}、{jp.Chapter}

其他格式变量：
- {name}：章节/卷名称
- {word_count}：字数

特殊功能：语言格式变量和 {name} 之间的下划线 `_` 会自动转换为中间点 `·`
